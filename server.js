...
if (!MONGODB_URI) {
  ...
  console.error('You must set MONGODB_URI in your Render.com service Environment variables before starting the app.');
  process.exit(1);
}
...
const Attendance = mongoose.model('Attendance', new Schema({
...
const Leave = mongoose.model('Leave', new Schema({
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      switch (action) {
        ...
        case 'update_leave':
          ...
          success_msg = 'Leave request updated.';
          break;
    ...
    if (page === 'dashboard') {
      ...
      if (req.query.fetch_course && req.query.fetch_subject) {
        ...
        const existing = await Score.find({ subject_id: req.query.fetch_subject });
        existing.forEach(s => { data.existing_scores[s.student_id] = s.score; });
        data.fetch_course = req.query.fetch_course;
        data.fetch_subject = req.query.fetch_subject;
      }
